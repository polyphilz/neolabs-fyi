...
export function LineageDetailPanel({ group, labs, onSelectLab, onClose }: Props) {
  if (!group) return null;

  const connected = labs
    .filter((lab) => lineageGroupsOf(lab).includes(group))
    .sort((a, b) => b.valuation.usdM - a.valuation.usdM || a.name.localeCompare(b.name));
  ...
  return (
    <aside className="drawer" aria-label={`${label} lineage details`}>
      ...
      <p className="drawer-known">Labs whose founding teams came out of {label}.</p>
